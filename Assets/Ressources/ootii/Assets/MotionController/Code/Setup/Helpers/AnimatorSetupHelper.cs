...
{    
    public static class AnimatorSetupHelper 
    {
        ...
        /// <summary>
        /// Ensures that the animator controller has a sufficient number of layers
        /// </summary>
        /// <param name="rAnimatorController"></param>
        /// <param name="rLayerIndex"></param>
        public static void EnsureAnimatorLayer(AnimatorController rAnimatorController, int rLayerIndex)
        {
            while (rAnimatorController.layers.Length <= rLayerIndex)
            {
                var lNewLayer = new AnimatorControllerLayer
                {
                    name = "Layer " + (rAnimatorController.layers.Length + 1),
                    stateMachine = new AnimatorStateMachine()
                };
                rAnimatorController.AddLayer(lNewLayer);

                EnsureLayerParameters(rAnimatorController, rLayerIndex);
            }
        }
        ...
        /// <summary>
        /// Remove the state with the specified name from the animator controller.
        /// </summary>
        /// <param name="rStateMachine"></param>
        /// <param name="rStateName"></param>
        public static void RemoveState(AnimatorStateMachine rStateMachine, string rStateName)
        {
            var lState = MotionControllerMotion.EditorFindState(rStateMachine, rStateName);
            if (lState == null) { return; }

            rStateMachine.RemoveState(lState);
        }
