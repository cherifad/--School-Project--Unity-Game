...
namespace com.ootii.Actors
{
    ...
    public class ActorController : BaseSystemController, ICharacterController
    {
        ...
        /// </summary>
        [Tooltip("Determines if the AC is updating and controlling movement and rotation.")]
        ...
        public bool UseTransformPosition
        {
            ...
            set
            {
                ...
                if (!_UseTransformPosition && _Transform != null)
                {
        ...
        public bool OrientToGround
        {
            ...
            set
            {
                ...
                if (_OrientToGround && _Transform != null)
                {
                ...
                else
                {
                    ...
                    if (_Transform != null && _Transform.up == Vector3.up)
                    {
        ...
        protected List<string> mOriginalBodyShapeDefinitions = new List<string>();
        ...
        void Start()
        {
            ...
            // Store a copy of the body shape definitions so that they can be restored later
            mOriginalBodyShapeDefinitions.Clear();
            mOriginalBodyShapeDefinitions.AddRange(mBodyShapeDefinitions);
        ...
        public void UpdateMovement(float rDeltaTime, int rUpdateIndex, Quaternion rFrameYaw, Quaternion rFrameTilt)
        {
            ...
            while (true)
            {
                ...
                {
                    bool lIsIdleOrFalling = true;
                    if (mAccumulatedVelocity.sqrMagnitude > 0f)
                    {
                        Vector3 lFinalAccumulatedForceVelocity = Vector3.Project(mAccumulatedVelocity, lActorUp);
                        lIsIdleOrFalling = Vector3.Dot(lFinalAccumulatedForceVelocity.normalized, lActorUp) < EPSILON;
                    }
...
//                        }
            ...
            {
                // TRT 04/28/2019: Don't force the ground penetration fix if we've got velocity
                // that is forcing up (ie a jump).
                bool lIsIdleOrFalling = true;
                if (mAccumulatedVelocity.sqrMagnitude > 0f)
                {
                    Vector3 lFinalAccumulatedForceVelocity = Vector3.Project(mAccumulatedVelocity, lActorUp);
        ...
        protected bool ProcessCollisions(Vector3 rSegmentPositionDelta, ref Vector3 rSegmentMovement, ref Vector3 rRemainingMovement)
        {
            ...
            for (int i = 0; i < BodyShapes.Count; i++)
            {
                ...
                BodyShapeHit[] lBodyShapeHits = BodyShapes[i].CollisionCastAll(rSegmentPositionDelta, lSegmentMovementDirection, rSegmentMovement.magnitude, _CollisionLayers, (mState.IsGrounded ? _MaxStepHeight : 0f));
        ...
        public void RestoreBodyShapes()
        {
            mBodyShapeDefinitions.Clear();
            mBodyShapeDefinitions.AddRange(mOriginalBodyShapeDefinitions);
            CreateBodyShapes();
        }
        ...
        public void DeserializeBodyShapes()
        {
            ...
            for (int i = 0; i < lBodyShapeDefCount; i++)
            {
                ...
                // the Type can still be obtained if the BodyShape was serialized as belonging to a different assembly
                bool lUpdateType;
                Type lType = AssemblyHelper.ResolveType(lTypeString, out lUpdateType);                
                if (lType == null) { continue; }

                // If don't have a body shape matching the type, we need to create one                
                ...
                if (lBodyShape != null)
                {
                    ...
                    // CDL 07/27/2018 - we need to update the AssemblyQualifiedName that it was serialized with
                    if (lUpdateType)
                    {
                        mBodyShapeDefinitions[i] = lBodyShape.Serialize();                        
                    }
                }               
            }
        }
        ...
        public void CreateDefaultShapes()
        {
            ...
            BodyCapsule lCapsule = new BodyCapsule
            {
                ...
                EndTransform = transform.FindTransform(HumanBodyBones.Head)
            };

            if (lCapsule.EndTransform == null)
            {
                lCapsule.EndTransform = transform.FindTransform("Head");
            }
