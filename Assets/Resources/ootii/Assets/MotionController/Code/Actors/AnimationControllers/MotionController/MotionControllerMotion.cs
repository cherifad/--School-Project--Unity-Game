...
namespace com.ootii.Actors.AnimationControllers
{
    ...
    public abstract class MotionControllerMotion : BaseObject, IMotionControllerMotion
    {
        ...
        /// Default constructor
        /// </summary>
        protected MotionControllerMotion(string rGUID)
        {
            _GUID = rGUID;
        }
        ...
        protected MotionControllerMotion(MotionController rController)
            : base()
        {
        ...
        {            
            ...
            //Debug.Log("[MotionControllerMotion] SerializeMotion: " + _Type);
            ...
            lStringBuilder.Append(", \"Form\" : \"" + _Form.ToString(CultureInfo.InvariantCulture) + "\"");
            lStringBuilder.Append(", \"Priority\" : \"" + _Priority.ToString(CultureInfo.InvariantCulture) + "\"");
            ...
            lStringBuilder.Append(", \"ReactivationDelay\" : \"" + _ReactivationDelay.ToString(CultureInfo.InvariantCulture) + "\"");
            lStringBuilder.Append(", \"ShowDebug\" : \"" + _ShowDebug.ToString() + "\"");
            lStringBuilder.Append(", \"Category\" : \"" + _Category.ToString(CultureInfo.InvariantCulture) + "\"");
        ...
        public virtual void DeserializeMotion(string rDefinition)
        {
            ...
            if (lTagsNode != null)
            {
                if (mTags != null && string.IsNullOrEmpty(lTagsNode.Value))
                {
                    mTags.Clear();
                    mTags = null;
                }
        ...
        /// </summary>
        /// <param name="rDisplayName">Label text</param>
        ...
        /// <param name="rStateMachineName"></param>
        ...
        protected virtual void SerializeSSM(string rParentID, AnimatorStateMachine rStateMachine, Vector3 rPosition, StringBuilder rText)
        {
            ...
            for (int i = 0; i < rStateMachine.entryTransitions.Length; i++)
            {
                ...
                {
                    if (!EntryTransitionIndexes.ContainsKey(rStateMachine.entryTransitions[i].destinationState)) { EntryTransitionIndexes.Add(rStateMachine.entryTransitions[i].destinationState, -1); }
                    EntryTransitionIndexes[rStateMachine.entryTransitions[i].destinationState] = EntryTransitionIndexes[rStateMachine.entryTransitions[i].destinationState] + 1;

                    SerializeEntryTransition(lStateMachineID, rStateMachine.entryTransitions[i], EntryTransitionIndexes[rStateMachine.entryTransitions[i].destinationState], rText);
                }
            }
        ...
        protected virtual void SerializeEntryTransition(string rParentID, AnimatorTransition rTransition, int rIndex, StringBuilder rText)
        {
            ...
            rText.AppendLine("    UnityEditor.Animations.AnimatorTransition " + lTransitionID + " = MotionControllerMotion.EditorFindEntryTransition(" + rParentID + ", lState_" + CleanName(rTransition.destinationState.GetInstanceID()) + ", " + rIndex + ");");
        ...
        protected virtual void GenerateScripts(AnimatorController rAnimatorController, int rAnimatorLayer, string rAnimatorSMName, string rStateNames)
        {
            ...
            for (int i = 0; i < mAnimationClipIDs.Count; i++)
            {
                ...
                if (lClipPath.IndexOf(mAnimationClips[i] + ".anim", StringComparison.Ordinal) >= 0)
                {
        ...
        /// States
        /// </summary>
        /// <param name="rParentStateMachine"></param>
        ...
        public static AnimationClip EditorFindAnimationClip(string rAssetPath, string rAssetName)
        {
            ...
            for (int i = 0; i < lAssets.Length; i++)
            {
                ...
                if (lClip != null)
                {
                    if (String.Compare(lClip.name, rAssetName, StringComparison.OrdinalIgnoreCase) == 0)
                    {
                        return lClip;
                    }
