    ...
    public class HealthInspector : StateBehaviorInspector
    {
        ...
        protected override Action GetDrawCallback()
        {
            ...
            baseCallback += () =>
            {
                ...
                if (Application.isPlaying && !string.IsNullOrEmpty(m_Health.HealthAttributeName) && selectedHealthNameIndex - 1 < m_AttributeManager.Attributes.Length) {
                ...
                if (Foldout("Audio")) {
                    ...
                    if (InspectorUtility.Foldout(target, "Take Damage")) {
                        EditorGUI.indentLevel++;
                        AudioClipSetInspector.DrawAudioClipSet(m_Health.TakeDamageAudioClipSet, ref m_ReorderableTakeDamageAudioClipsList, OnTakeDamageAudioClipDraw, OnTakeDamageAudioClipListAdd, OnTakeDamageAudioClipListRemove);
                        EditorGUI.indentLevel--;
                    }
                    if (InspectorUtility.Foldout(target, "Heal")) {
                        EditorGUI.indentLevel++;
                        AudioClipSetInspector.DrawAudioClipSet(m_Health.HealAudioClipSet, ref m_ReorderableHealAudioClipsList, OnHealAudioClipDraw, OnHealAudioClipListAdd, OnHealAudioClipListRemove);
                        EditorGUI.indentLevel--;
                    }
                    if (InspectorUtility.Foldout(target, "Death")) {
                        EditorGUI.indentLevel++;
                        AudioClipSetInspector.DrawAudioClipSet(m_Health.DeathAudioClipSet, ref m_ReorderableDeathAudioClipsList, OnDeathAudioClipDraw, OnDeathAudioClipListAdd, OnDeathAudioClipListRemove);
                        EditorGUI.indentLevel--;
                    }
                    EditorGUI.indentLevel--;
                }
        ...
        {
            AudioClipSetInspector.OnAudioClipDraw(rect, index, m_Health.TakeDamageAudioClipSet, null);
        }
        ...
        {
            AudioClipSetInspector.OnAudioClipDraw(rect, index, m_Health.HealAudioClipSet, null);
        }
        ...
        {
            AudioClipSetInspector.OnAudioClipDraw(rect, index, m_Health.DeathAudioClipSet, null);
        }
