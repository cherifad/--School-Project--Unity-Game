...
namespace Opsive.UltimateCharacterController.Editor.Utility
{
    ...
    public static class AnimatorBuilder
    {
        ...
        public static void GenerateAnimatorCode(AnimatorController animatorController, string parameterName, float parameterValue, object parentObject, string baseDirectory)
        {
            var generatedCode = new StringBuilder();
            var generatedStateMachineCode = false;
            ...
            for (int i = 0; i < animatorController.layers.Length; ++i) {
                ...
                if (stateMachines.Count > 0) {
                    generatedStateMachineCode = true;

                    // The state machine should start fresh.
                    var baseStateMachine = stateMachines[0];
                    var baseStateMachineName = "baseStateMachine" + i;
                    generatedCode.AppendLine("\t\t\tvar " + baseStateMachineName + " = animatorController.layers[" + i + "].stateMachine;");
                    generatedCode.AppendLine();
                    generatedCode.AppendLine("\t\t\t// The state machine should start fresh.");
                    generatedCode.AppendLine("\t\t\tfor (int i = 0; i < animatorController.layers.Length; ++i) {");
                    ...
                    for (int j = 0; j < transitions.Count; ++j) {
                        GenerateTransition(baseStateMachineName, transitions[j].Transition, false, generatedCode);
            ...
            if (generatedStateMachineCode) {
                ...
                if (path.Length != 0 && Application.dataPath.Length < path.Length) {
                    ...
                    if (File.Exists(path)) {
                        ...
                        if (parentObject is Character.Abilities.Ability) {
                            fileString.AppendLine("using Opsive.UltimateCharacterController.Character.Abilities;");
                        } else if (parentObject is Character.Abilities.Items.ItemAbility) {
                            fileString.AppendLine("using Opsive.UltimateCharacterController.Character.Abilities.Items;");
                        }
                        ...
                        fileString.AppendLine("\t/// </summary>");
                        fileString.AppendLine("\t[InspectorDrawer(typeof(" + parentName + "))]");
                    ...
                    fileString.AppendLine("\t\t// --- Do NOT change any code after this otherwise it will be removed when the animator is generated again. ---");
                    fileString.AppendLine("\t\t// ------------------------------------------------------------------------------------------------------------");
                    fileString.AppendLine();
                    fileString.AppendLine("\t\t/// <summary>");
                    ...
                    fileString.AppendLine("\t\t/// </summary>");
                    fileString.AppendLine("\t\t/// <param name=\"animatorController\">The editor AnimatorController to add the states to.</param>");
                    fileString.AppendLine("\t\tpublic override void BuildAnimator(AnimatorController animatorController)");
                    fileString.AppendLine("\t\t{");
        ...
        private static void GenerateStateMachine(string parentStateMachineName, ChildAnimatorStateMachine childStateMachine, StringBuilder generatedCode)
        {
            ...
            if (stateMachine.anyStateTransitions.Length > 0) {
                ...
                for (int i = 0; i < stateMachine.anyStateTransitions.Length; ++i) {
                    GenerateTransition(uniqueStateMachineName, stateMachine.anyStateTransitions[i], false, generatedCode);
                    generatedCode.AppendLine();
                }
            }
            ...
            if (stateMachine.entryTransitions.Length > 0) {
                ...
                for (int i = 0; i < stateMachine.entryTransitions.Length; ++i) {
                    GenerateTransition(uniqueStateMachineName, stateMachine.entryTransitions[i], false, generatedCode);
                    generatedCode.AppendLine();
                }
            }
            ...
            if (stateMachine.states.Length > 0) {
                generatedCode.AppendLine("\t\t\t// States Transitions.");
                for (int i = 0; i < stateMachine.states.Length; ++i) {
                    var state = stateMachine.states[i].state;
                    var uniqueStateName = UniqueName(stateMachine.states[i].state);
                    for (int j = 0; j < state.transitions.Length; ++j) {
                        GenerateTransition(uniqueStateName, state.transitions[j], true, generatedCode);
                        generatedCode.AppendLine();
                    }
                }
            }
        ...
        /// <param name="indentLevel">The number of indentations for the generated code.</param>
        private static void GenerateTransition(string stateMachineName, AnimatorTransitionBase transition, bool isStateTransition, StringBuilder generatedCode)
        {
            var destinationStateName = UniqueName(transition.destinationState);
            var transitionName = UniqueName(transition);
            if (isStateTransition) {
                generatedCode.AppendLine("\t\t\tvar " + transitionName + " = " + stateMachineName + ".AddTransition(" + destinationStateName + ");");
                ...
                generatedCode.AppendLine("\t\t\tvar " + transitionName + " = " + stateMachineName + ".AddEntryTransition(" + destinationStateName + ");");
            }
        ...
        public static void AddParameters(AnimatorController animatorController)
        {
            ...
            for (int i = 0; i < 2; ++i) {
                ...
                parameterName = string.Format("Slot{0}ItemSubstateIndexChange", i);
                if (!HasParameter(animatorController, parameterName)) {
