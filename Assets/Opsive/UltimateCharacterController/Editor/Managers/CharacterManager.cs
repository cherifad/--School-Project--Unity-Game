    ...
    public class CharacterManager : Manager
    {
        ...
        private void DrawNewCharacter()
        {
            ...
            if (GUILayout.Button("Build Character")) {
                ...
                if (!string.IsNullOrEmpty(AssetDatabase.GetAssetPath(m_AddCharacter))) {
                    var name = m_AddCharacter.name;
                    m_AddCharacter = GameObject.Instantiate(m_AddCharacter) as GameObject;
                    m_AddCharacter.name = name;
                }

                CharacterBuilder.BuildCharacter(m_AddCharacter, m_FirstPersonMovementType, m_ThirdPersonMovementType, m_StartFirstPersonPerspective, m_AddFirstPersonHiddenObjects, m_InvisibleShadowCaster, m_AddAIAgent);
                CharacterBuilder.BuildCharacterComponents(m_AddCharacter, m_AddAnimator, m_AddAnimatorController, m_AddAIAgent,m_AddItems, m_ExistingItemCollection, !string.IsNullOrEmpty(m_FirstPersonMovementType), m_AddHealth, m_AddUnityIK, m_AddFootEffects);
        ...
        private void DrawExistingCharacter()
        {
                ...
                if (m_ExistingCharacter != null) {
                    ...
                    if (!HasEssentials(m_ExistingCharacter, m_ExistingPerspective == Perspective.Both)) {
                        ...
                        if (GUILayout.Button("Add")) {
                            CharacterBuilder.AddEssentials(m_ExistingCharacter, m_ExistingPerspective == Perspective.Both, m_ExistingAIAgent);
                        }
                    }
            ...
            if (GUILayout.Button("Update Character")) {
                if (m_ExistingPerspective != CurrentPerspective(m_ExistingCharacter)) {
                    ...
                    var perspectiveMonitor = m_ExistingCharacter.GetComponent<ThirdPersonController.Character.PerspectiveMonitor>();
                    if (m_ExistingPerspective == Perspective.Both) {
                        if (perspectiveMonitor == null) {
                            m_ExistingCharacter.AddComponent<ThirdPersonController.Character.PerspectiveMonitor>();
                        }
                ...
                if (m_ExistingAnimator != HasAnimator(m_ExistingCharacter)) {
                    if (m_ExistingAnimator) {
                        CharacterBuilder.AddAnimator(m_ExistingCharacter, m_ExistingAnimatorController);
                    } else {
        ...
        {
#if THIRD_PERSON_CONTROLLER
            if (usePerspectiveMonitor && character.GetComponent<ThirdPersonController.Character.PerspectiveMonitor>() == null) {
                return false;
            }
#endif
            return character.GetComponent<Rigidbody>() != null && character.GetComponent<UltimateCharacterLocomotion>() != null &&
                    character.GetComponent<UltimateCharacterLocomotionHandler>() && character.GetComponent<LayerManager>() != null;
        }
...
#if THIRD_PERSON_CONTROLLER
            hasBothComponents = m_ExistingCharacter.GetComponentInChildren<ThirdPersonController.Character.PerspectiveMonitor>() != null;
#endif
...
#if FIRST_PERSON_CONTROLLER
            if (checkFirstPersonObjects && character.GetComponentInChildren<FirstPersonController.Character.FirstPersonObjects>() == null) {
                return false;
            }
#endif
