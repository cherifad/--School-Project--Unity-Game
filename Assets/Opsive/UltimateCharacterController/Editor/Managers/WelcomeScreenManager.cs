    ...
    public class WelcomeScreenManager : Manager
    {
        ...
        public override void OnGUI()
        {
            EditorGUILayout.LabelField(string.Format("Thank you for purchasing the {0}.\nThe resources below will help you get the most out of the controller.", 
                                            UltimateCharacterController.Utility.AssetInfo.Name), InspectorUtility.WordWrapLabelCenter);
            ...
            GUILayout.Label(m_DocumentationTexture, InspectorUtility.CenterLabel, GUILayout.Width(m_DocumentationTexture.width), GUILayout.Height(m_DocumentationTexture.height));
            ...
            var selected = GUILayout.SelectionGrid(-1, 
                ...
                2, 
                InspectorUtility.CenterLabel, GUILayout.Width(m_IntegrationsTexture.width * 2));
            if (selected != -1) {
                switch(selected) {
                    case 0:
                        Application.OpenURL("https://www.opsive.com/integrations/?pid=923");
                        break;
                    case 1:
            ...
            GUILayout.BeginHorizontal();
            EditorGUILayout.LabelField("Version: " + UltimateCharacterController.Utility.AssetInfo.Version);
            try {
                ...
                if (!string.IsNullOrEmpty(m_MainManagerWindow.LatestVersion) && version.CompareTo(new Version(m_MainManagerWindow.LatestVersion)) < 0) {
                    EditorGUILayout.LabelField(" New version available: " + m_MainManagerWindow.LatestVersion);
                }
            } catch (Exception /*e*/) { }
            GUILayout.EndHorizontal();
        }
