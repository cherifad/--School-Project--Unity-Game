...
namespace Opsive.UltimateCharacterController.Editor.Managers
{
    ...
    public class MainManagerWindow : EditorWindow
    {
        ...
        {
            get { return EditorPrefs.GetString("Opsive.UltimateCharacterController.Editor.LatestVersion", UltimateCharacterController.Utility.AssetInfo.Version); }
        ...
        private GUIStyle ManagerTitle
        {
            get
            {
                if (m_ManagerTitle == null) {
                    m_ManagerTitle = new GUIStyle(InspectorUtility.CenterBoldLabel);
        ...
        private void BuildManagerItems()
        {
            ...
            for (int i = 0; i < assemblies.Length; ++i) {
                ...
                for (int j = 0; j < assemblyTypes.Length; ++j) {
                    ...
                    if (assemblyTypes[j].GetCustomAttributes(typeof(ManagerMenuItem), true).Length > 0) {
                        var managerMenuItem = assemblyTypes[j].GetCustomAttributes(typeof(ManagerMenuItem), true)[0] as ManagerMenuItem;
                        index = managerMenuItem.Index;
                    }
            ...
            for (int i = 0; i < managerTypes.Length; ++i) {
                ...
                if (managers[i].GetCustomAttributes(typeof(ManagerMenuItem), true).Length > 0) {
                    var managerMenuItem = managerTypes[i].GetCustomAttributes(typeof(ManagerMenuItem), true)[0] as ManagerMenuItem;
                    name = managerMenuItem.Name;
                }
    ...
    /// <summary>
    /// Attribute which specifies the name and ordering of the menu items.
    /// </summary>
    [AttributeUsage(AttributeTargets.Class, AllowMultiple = false, Inherited = false)]
    public class ManagerMenuItem : Attribute
    {
        private string m_Name;
        private int m_Index;
        public string Name { get { return m_Name; } }
        public int Index { get { return m_Index; } }
        public ManagerMenuItem(string name, int index) { m_Name = name; m_Index = index; }
    }
}