...
namespace Opsive.UltimateCharacterController.Traits
{
    ...
    public class Health : StateBehavior
    {
        ...
        /// <summary>
        /// Gets the attributes from the AttributeManager.
        /// </summary>
        ...
        public virtual void Damage(float amount, Vector3 position, Vector3 direction, float forceMagnitude, int frames, float radius, GameObject attacker, Collider hitCollider)
        {
            ...
            if (radius == 0 && direction != Vector3.zero && hitCollider != null) {
                ...
                if (m_ColliderHitboxMap != null && m_ColliderHitboxMap.Count > 0) {
                    ...
                    if (hitCount > 0) {
                        System.Array.Sort(m_RaycastHits, 0, hitCount, m_RaycastHitComparer);
                        for (int i = 0; i < hitCount; ++i) {
                            if (m_RaycastHits[i].collider == hitCollider) {
                                continue;
                            }
                            // A new collider has been found - stop iterating so the new collider can be used.
                            hitCollider = m_RaycastHits[i].collider;
                            break;
                        }
                    }
            ...
            if (m_OnDamageEvent != null && m_OnDamageEvent.GetPersistentEventCount() > 0) {
        ...
        protected virtual void Die(Vector3 position, Vector3 force, GameObject attacker)
        {
            ...
            if (m_OnDeathEvent != null && m_OnDeathEvent.GetPersistentEventCount() > 0) {
        ...
        public virtual bool Heal(float amount)
        {
            ...
            if (m_OnHealEvent != null && m_OnHealEvent.GetPersistentEventCount() > 0) {
