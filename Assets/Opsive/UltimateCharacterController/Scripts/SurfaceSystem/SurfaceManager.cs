...
namespace Opsive.UltimateCharacterController.SurfaceSystem
{
    ...
    public class SurfaceManager : MonoBehaviour
    {
        ...
        {
            return Instance.SpawnEffectInternal(hit, surfaceImpact, gravityDirection, timeScale, originator);
        }
        ...
        /// <param name="originator">The object which spawned the effect.</param>
        /// <returns>True if the effect was spawned.</returns>
        private bool SpawnEffectInternal(RaycastHit hit, SurfaceImpact surfaceImpact, Vector3 gravityDirection, float timeScale, GameObject originator)
        {
            SurfaceType surfaceType = null;
            var spawnDecals = false;
            var surfaceEffect = GetSurfaceEffect(hit, surfaceImpact, ref surfaceType, ref spawnDecals);
        ...
        {
            return Instance.SpawnEffectInternal(hit, surfaceImpact, gravityDirection, timeScale, originator, footprintDirection, flipFootprint);
        }
        ...
        /// <param name="originator">The object which spawned the effect.</param>
        /// <returns>True if the effect was spawned.</returns>
        private bool SpawnEffectInternal(RaycastHit hit, SurfaceImpact surfaceImpact, Vector3 gravityDirection, float timeScale, GameObject originator, Vector3 footprintDirection, bool flipFootprint)
        {
            SurfaceType surfaceType = null;
            var spawnDecals = false;
            var surfaceEffect = GetSurfaceEffect(hit, surfaceImpact, ref surfaceType, ref spawnDecals);
        ...
        private SurfaceEffect GetSurfaceEffect(RaycastHit hit, SurfaceImpact surfaceImpact, ref SurfaceType surfaceType, ref bool spawnDecals)
        {
            surfaceType = GetSurfaceType(hit);
            spawnDecals = ShouldSpawnDecals(hit.collider);
        ...
        private SurfaceType GetSurfaceType(RaycastHit hit)
        {
            ...
            var surfaceIdentifier = GetSurfaceIdentifier(hit.collider);
        ...
        private SurfaceType GetComplexSurfaceType(RaycastHit hit)
        {
            // GetHitMaterial will only return a value if the hit collider is a MeshCollider.
            var material = GetHitMaterial(hit);
            if (material == null) {
                return null;
            }

            return GetComplexSurfaceType(material, hit);
        }
        ...
        private Material GetHitMaterial(RaycastHit hit)
        {
            ...
            var renderer = GetRenderer(hit.collider);
        ...
        private SurfaceType GetComplexSurfaceType(Material material, RaycastHit hit)
        {
            if (material == null) {
                return null;
            }
            ...
            if (!(hit.collider is MeshCollider)) {
                Debug.LogWarning("Warning: Surface UV regions on " + hit.collider.name + " only support MeshColliders.");
                return null;
            }
        ...
        private SurfaceType GetTerrainSurfaceType(RaycastHit hit)
        {
            if (!m_HasTerrain) {
                return null;
            }

            var texture = GetTerrainTexture(hit.collider, hit.point);
            if (texture == null) {
                return null;
            }
