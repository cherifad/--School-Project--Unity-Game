...
namespace Opsive.UltimateCharacterController.Utility.Builders
{
    ...
    public static class CharacterBuilder
    {
        ...
        public static void BuildCharacter(GameObject character, string firstPersonMovementType, string thirdPersonMovementType, bool startFirstPersonPerspective,
            GameObject[] firstPersonHiddenObjects, Material invisibleShadowCasterMaterial, bool aiAgent)
        {
            AddEssentials(character, !string.IsNullOrEmpty(firstPersonMovementType) && !string.IsNullOrEmpty(thirdPersonMovementType), aiAgent);
        ...
        /// <param name="aiAgent">Is the character an AI agent?</param>
        public static void AddEssentials(GameObject character, bool addPerspectiveMonitor, bool aiAgent)
        {
            if (!aiAgent) {
                character.tag = "Player";
            }
            character.layer = LayerManager.Character;
            if (character.GetComponent<LayerManager>() == null) {
                character.AddComponent<LayerManager>();
            }
            ...
            if (collider != null && animator != null && animator.isHuman) {
            ...
            if (addPerspectiveMonitor && character.GetComponent<ThirdPersonController.Character.PerspectiveMonitor>() == null) {
                character.AddComponent<ThirdPersonController.Character.PerspectiveMonitor>();
            }
#endif
        ...
        public static void RemoveEssentials(GameObject character)
        {
            ...
            var layerManager = character.GetComponent<LayerManager>();
        ...
        public static void BuildCharacterComponents(GameObject character, bool addAnimator, RuntimeAnimatorController animatorController, bool aiAgent, bool addItems, ItemCollection itemCollection, bool firstPersonItems, bool addHealth, bool addUnityIK, bool addFootEffects)
        {
            if (addAnimator) {
                AddAnimator(character, animatorController);
            }
            if (aiAgent) {
                AddAIAgent(character);
            } else { // If not an AI agent, then a player controlled character.
        ...
        /// <param name="animatorController">A reference to the animator controller.</param>
        public static void AddAnimator(GameObject character, RuntimeAnimatorController animatorController)
        {
            Animator animator;
