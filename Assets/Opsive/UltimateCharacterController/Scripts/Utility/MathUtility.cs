...
namespace Opsive.UltimateCharacterController.Utility
{
    ...
    public class MathUtility
    {
        ...
        public static void CapsuleColliderEndCaps(CapsuleCollider capsuleCollider, Vector3 position, Quaternion rotation, out Vector3 firstEndCap, out Vector3 secondEndCap)
        {
            ...
            firstEndCap = TransformPoint(position, rotation, capsuleCollider.center + direction * (-(capsuleCollider.height * 0.5f) + capsuleCollider.radius));
            secondEndCap = firstEndCap + (rotation * direction) * (capsuleCollider.height - capsuleCollider.radius * 2);
        }
        ...
        public static Vector3 ClosestPointOnCollider(Transform transform, Collider collider, Vector3 point, Vector3 moveDirection, bool sphereCheck, bool lowerPoint)
        {
            if (collider is CapsuleCollider) {
                ...
                return ClosestPointOnSphere(transform, point, collider.transform.TransformPoint(sphereCollider.center), sphereCollider.radius, sphereCheck, lowerPoint);
            }
        }
        ...
        private static Vector3 ClosestPointOnCapsule(Transform transform, CapsuleCollider capsuleCollider, Vector3 point, Vector3 moveDirection, bool sphereCheck, bool lowerPoint)
        {
            ...
            var capsuleLength = ((capsuleCollider.height * 0.5f) - capsuleCollider.radius);
            ...
            if (segment >= 0 && segment <= 1) { // On cylinder.
                ...
                var pointDirection = (point - closestPoint).normalized * capsuleCollider.radius;
                if (lowerPoint) {
                ...
                return ClosestPointOnSphere(transform, point, sphereCenter, capsuleCollider.radius, sphereCheck, lowerPoint);
            }
        }
