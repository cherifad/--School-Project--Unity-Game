...
namespace Opsive.UltimateCharacterController.Utility
{
    ...
    public class Serialization
    {
                ...
                for (int i = 0; i < properties.Length; ++i) {
                    ...
                    if (getMethod != null) {
                        ...
                        if (value == null && !Serializer.IsSerializedType(properties[i].PropertyType) && !typeof(IList).IsAssignableFrom(properties[i].PropertyType) && 
                            ...
                            var setMethod = properties[i].GetSetMethod((visibility != MemberVisibility.Public && visibility != MemberVisibility.PublicDefault && visibility != MemberVisibility.AllPublic));
        ...
        {
            var getMethod = property.GetGetMethod((visibility != MemberVisibility.Public && visibility != MemberVisibility.PublicDefault && visibility != MemberVisibility.AllPublic));
            ...
            var setMethod = property.GetSetMethod((visibility != MemberVisibility.Public && visibility != MemberVisibility.PublicDefault && visibility != MemberVisibility.AllPublic));
            if (setMethod == null) {
                return null;
            }
        ...
        private static void GetSerializedFields(Type type, ref List<FieldInfo> fieldList, BindingFlags flags, MemberVisibility visibility)
        {
            ...
            for (int i = 0; i < fields.Length; ++i) {
                ...
                // - The visibility is public default but the field has the NonDefaultSerialized attribute.
                // - The visiblity is snapshot but the field doesn't have the Snapshot attribute.
                // - The field is of type Serialization.
                if ((visibility!= MemberVisibility.AllPublic && UnityEngineUtility.HasAttribute(fields[i], typeof(NonSerializedAttribute))) ||
                    ((visibility == MemberVisibility.Public || visibility == MemberVisibility.PublicDefault) && (fields[i].IsPrivate || fields[i].IsFamily) && 
                                                !UnityEngineUtility.HasAttribute(fields[i], typeof(SerializeField))) ||
                    (visibility == MemberVisibility.PublicDefault && UnityEngineUtility.HasAttribute(fields[i], typeof(NonDefaultSerialized))) ||
                    (visibility == MemberVisibility.Snapshot && !UnityEngineUtility.HasAttribute(fields[i], typeof(Snapshot))) ||
                    fields[i].FieldType == typeof(Serialization)) {
                    continue;
                }
        ...
        public static PropertyInfo[] GetSerializedProperties(Type type, MemberVisibility visibility)
        {
            ...
            if (!s_SerializedPropertiesMaps[mapIndex].TryGetValue(type, out properties)) {
                ...
                if (visibility != MemberVisibility.Public && visibility != MemberVisibility.PublicDefault && visibility != MemberVisibility.AllPublic) {
                ...
                for (int i = 0; i < properties.Length; ++i) {
                    ...
                    // - The visiblity is PublicDefault but the property has the NonDefaultSerialized attribute.
                    ...
                    if ((visibility != MemberVisibility.Snapshot && visibility != MemberVisibility.AllPublic && 
                        ...
                        (visibility == MemberVisibility.PublicDefault && UnityEngineUtility.HasAttribute(properties[i], typeof(NonDefaultSerialized))) ||
    ...
    public enum MemberVisibility
    {
        ...
        PublicDefault,  // Only returns public members that can be serialized to the default preset.
    ...
    /// <summary>
    /// Attribute which prevents preset serialization on the property for the default state.
    /// </summary>
    [AttributeUsage(AttributeTargets.Property, AllowMultiple = false, Inherited = false)]
    public class NonDefaultSerialized : Attribute
    {
        // Intentionally left blank.
    }
}