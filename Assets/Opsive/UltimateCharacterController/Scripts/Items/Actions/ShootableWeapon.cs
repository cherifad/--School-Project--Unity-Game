...
namespace Opsive.UltimateCharacterController.Items.Actions
{
    ...
    public class ShootableWeapon : UsableItem, IReloadableItem
    {
        ...
        public override int GetItemSubstateIndex()
        {
            ...
            if (m_ConsumableItemType != null && m_ClipRemaining == 0) {
        ...
        public override bool CanUseItem(ItemAbility itemAbility, UseAbilityState abilityState)
        {
            ...
            if (m_Reloading && m_ClipRemaining == 0) {
                return false;
            }
        ...
        private void HitscanFire(float strength)
        {
            ...
            if (hitCount > 0) {
                ...
                System.Array.Sort(m_HitscanRaycastHits, 0, hitCount, m_RaycastHitComparer);
                for (int i = 0; i < hitCount; ++i) {
                    var hitGameObject = m_HitscanRaycastHits[i].transform.gameObject;
                    ...
                    EventHandler.ExecuteEvent(m_HitscanRaycastHits[i].transform.gameObject, "OnObjectImpact", damageAmount, m_HitscanRaycastHits[i].point, fireDirection * m_ImpactForce * strength, m_Character, m_HitscanRaycastHits[i].collider);
                    if (m_OnHitscanImpactEvent != null && m_OnHitscanImpactEvent.GetPersistentEventCount() > 0) {
                        m_OnHitscanImpactEvent.Invoke(damageAmount, m_HitscanRaycastHits[i].point, fireDirection * m_ImpactForce * strength, m_Character);
                    }
                    ...
                    if (m_Tracer != null) {
                        Scheduler.ScheduleFixed(m_TracerSpawnDelay, AddHitscanTracer, m_HitscanRaycastHits[i].point);
                    }
                    ...
                    SurfaceManager.SpawnEffect(m_HitscanRaycastHits[i], m_SurfaceImpact, m_CharacterLocomotion.GravityDirection, m_CharacterLocomotion.TimeScale, m_Item.GetVisibleObject());
