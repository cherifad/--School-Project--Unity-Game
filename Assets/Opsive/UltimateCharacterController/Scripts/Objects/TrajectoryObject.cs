...
{
    /// <summary>
    /// A trajectory object follows a deterministic parabolic curve and can be simuated using the SimulateTrajectory method.
    /// </summary>
    public class TrajectoryObject : MonoBehaviour, IForceObject
    {
        ...
        public enum BounceMode
        {
            ...
            RandomReflect   // Reflect in a random direction. This mode will make the object nondeterministic but for visual only objects such as shells this is preferred.
        }
        ...
        private bool OverlapCast(Vector3 position, Quaternion rotation)
        {
            ...
            if (m_Collider is SphereCollider) {
                ...
                hit = Physics.OverlapSphereNonAlloc(MathUtility.TransformPoint(position, m_Transform.rotation, sphereCollider.center), sphereCollider.radius * m_Scale, m_ColliderHit, m_ImpactLayers, QueryTriggerInteraction.Ignore);
                ...
                hit = Physics.OverlapCapsuleNonAlloc(startEndCap, endEndCap, capsuleCollider.radius * m_Scale, m_ColliderHit, m_ImpactLayers, QueryTriggerInteraction.Ignore);
            } else if (m_Collider is BoxCollider) {
                var boxCollider = m_Collider as BoxCollider;
                hit = Physics.OverlapBoxNonAlloc(MathUtility.TransformPoint(position, m_Transform.rotation, boxCollider.center), boxCollider.size / 2, m_ColliderHit, m_Transform.rotation, m_ImpactLayers, QueryTriggerInteraction.Ignore);
            }
        ...
        protected void InitializeComponentReferences()
        {
            ...
            // The transform's scale affects the collisions
            m_Scale = m_Transform.lossyScale.x;
            if (m_Transform.lossyScale.y > m_Scale) m_Scale = m_Transform.lossyScale.y;
            if (m_Transform.lossyScale.z > m_Scale) m_Scale = m_Transform.lossyScale.z;
        }
        ...
        private bool SingleCast(Vector3 position, Quaternion rotation, Vector3 direction)
        {
            ...
            if (m_Collider is SphereCollider) {
                var sphereCollider = m_Collider as SphereCollider;
                hit = Physics.SphereCast(position, sphereCollider.radius * m_Scale, direction.normalized, out m_RaycastHit, direction.magnitude + c_ColliderSpacing, m_ImpactLayers, QueryTriggerInteraction.Ignore);
            } else if (m_Collider is CapsuleCollider) {
                var capsuleCollider = m_Collider as CapsuleCollider;
                Vector3 startEndCap, endEndCap;
                MathUtility.CapsuleColliderEndCaps(capsuleCollider, position, rotation, out startEndCap, out endEndCap);
                hit = Physics.CapsuleCast(startEndCap, endEndCap, capsuleCollider.radius * m_Scale, direction.normalized, out m_RaycastHit, direction.magnitude + c_ColliderSpacing, m_ImpactLayers, QueryTriggerInteraction.Ignore);
            } else if (m_Collider is BoxCollider) {
                var boxCollider = m_Collider as BoxCollider;
        ...
        private void Rotate(Vector3 position, ref Quaternion rotation)
        {
            ...
            if (m_Collider is CapsuleCollider) {
                ...
                hitCount = Physics.OverlapCapsuleNonAlloc(startEndCap, endEndCap, capsuleCollider.radius, m_ColliderHit, m_ImpactLayers, QueryTriggerInteraction.Ignore);
            } else if (m_Collider is BoxCollider) {
                var boxCollider = m_Collider as BoxCollider;
                hitCount = Physics.OverlapBoxNonAlloc(MathUtility.TransformPoint(position, m_Transform.rotation, boxCollider.center), boxCollider.size / 2, m_ColliderHit, m_Transform.rotation, m_ImpactLayers, QueryTriggerInteraction.Ignore);
            }
