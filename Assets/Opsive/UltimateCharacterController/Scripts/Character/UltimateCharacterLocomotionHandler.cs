...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public class UltimateCharacterLocomotionHandler : StateBehavior
    {
        ...
        private void Update()
        {
            // The input should be retrieved within Update. The DeterministicObjectManager will then move the character according to the input.
            ...
            DeterministicObjectManager.SetCharacterMovementInput(m_CharacterLocomotion.DeterministicObjectIndex, m_HorizontalMovement, m_ForwardMovement);
        ...
        private void UpdateAbilityInput(Ability[] abilities)
        {
            if (abilities != null) {
                ...
                for (int i = 0; i < abilities.Length; ++i) {
                    ...
                    var abilityStarted = false;
                    if (!abilities[i].IsActive || abilities[i].CanReceiveMultipleStarts) {
                        ...
                        if (abilities[i].CanInputStartAbility(m_PlayerInput)) {
                            abilityStarted = TryStartAbility(abilities[i]);
                        }
                    }

                    // Use a separate if statement because the ability may be active while receiving multiple starts. If the if was an else then a
                    // active multiple start ability would never be checked to see if it can stop.
                    if (!abilityStarted && abilities[i].IsActive) {
        ...
        private void OnEnableGameplayInput(bool enable)
        {
            ...
            if (!m_InputEnabled) {
                ...
                DeterministicObjectManager.SetCharacterMovementInput(m_CharacterLocomotion.DeterministicObjectIndex, 0, 0);
            }
        }
