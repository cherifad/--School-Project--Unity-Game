...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public class UltimateCharacterLocomotion : CharacterLocomotion
    {
        ...
        private int m_DeterministicObjectIndex = -1;
        ...
        private bool m_ApplyAnimatorMove;
        private bool m_Aiming;

        private int[] m_ItemSlotStateIndex;
        private int[] m_ItemSlotSubstateIndex;

        [NonSerialized] public int DeterministicObjectIndex { get { return m_DeterministicObjectIndex; } set { m_DeterministicObjectIndex = value; } }
        public ILookSource LookSource { get { return m_LookSource; } }
        public MovementType[] MovementTypes { get { if (m_MovementTypes == null) { DeserializeMovementTypes(); } return m_MovementTypes; }
            set
            {
        ...
        public Ability[] Abilities { get { if (m_Abilities == null) { DeserializeAbilities(); } return m_Abilities; } set { m_Abilities = value;
        ...
        public ItemAbility[] ItemAbilities { get { if (m_ItemAbilities == null) { DeserializeItemAbilities(); } return m_ItemAbilities; } set { m_ItemAbilities = value;
        ...
        public Effect[] Effects { get { if (m_Effects == null) { DeserializeEffects(); } return m_Effects; } set { m_Effects = value; } }
        ...
        [NonDefaultSerialized] public override float TimeScale
        {
            ...
            set
            {
                ...
                if (base.TimeScale != value) {
                    ...
                    if (m_OnChangeTimeScaleEvent != null && m_OnChangeTimeScaleEvent.GetPersistentEventCount() > 0) {
        ...
        [NonSerialized] public bool Moving { get { return m_Moving || m_InputVector.sqrMagnitude > 0.001f; } }
        ...
        public void SetMovementType(System.Type type)
        {
            ...
            if (m_MovementType != null && Application.isPlaying) {
                ...
                EventHandler.ExecuteEvent(m_GameObject, "OnChangeMovementType", m_MovementType, false);

                if (m_OnMovementTypeActiveEvent != null && m_OnMovementTypeActiveEvent.GetPersistentEventCount() > 0) {
            ...
            if (Application.isPlaying) {
                ...
                EventHandler.ExecuteEvent(m_GameObject, "OnChangeMovementType", m_MovementType, true);
        ...
        {
            var prevIndex = m_DeterministicObjectIndex;

            // The DeterministicObjectManager is responsible for calling the move method.
            m_DeterministicObjectIndex = DeterministicObjectManager.RegisterCharacter(this);
            ...
            if (abilities != null) {
                for (int i = 0; i < abilities.Length; ++i) {
                    ...
                    if (!abilities[i].IsActive) {
                        ...
                        } else if (!(abilities[i] is ItemAbility) && abilities[i].StartType != Ability.AbilityStartType.Manual) {
        ...
        protected override bool UpdateGroundState(bool grounded, bool sendEvents)
        {
            ...
            if (groundedStatusChanged) {
                ...
                if (sendEvents) {
                    ...
                    if (m_OnGroundedEvent != null && m_OnGroundedEvent.GetPersistentEventCount() > 0) {
                ...
                if (m_Grounded) {
                    if (sendEvents && m_MaxHeight != float.NegativeInfinity) {
                        ...
                        if (m_OnLandEvent != null && m_OnLandEvent.GetPersistentEventCount() > 0) {
        ...
        protected override bool UpdateMovingPlatformTransform(Transform hitTransform, bool groundUpdate)
        {
            ...
            if (movingPlatformChanged) {
                ...
                if (m_OnChangeMovingPlatformsEvent != null && m_OnChangeMovingPlatformsEvent.GetPersistentEventCount() > 0) {
        ...
        protected override void UpdateAnimator()
        {
            ...
            m_AnimatorMonitor.SetHorizontalMovementParameter(m_InputVector.x);
            m_AnimatorMonitor.SetForwardMovementParameter(m_InputVector.y);
            if (m_LookSource != null) {
                m_AnimatorMonitor.SetPitchParameter(m_LookSource.Pitch, 0);
            }
            m_AnimatorMonitor.SetYawParameter(m_YawAngle * m_YawMultiplier);
            m_AnimatorMonitor.SetMovingParameter(m_MovingParameter);

            // The ability parameters should only be updated once each move call.
            UpdateDirtyAbilityAnimatorParameters();

            m_ApplyAnimatorMove = true;
            m_AnimatorMonitor.UpdateAnimator();
#if UNITY_EDITOR
            m_ApplyAnimatorMove = m_AnimatorMonitor.DebugAnimatorController;
#else
            m_ApplyAnimatorMove = false;
#endif
        }
        ...
        /// <summary>
        /// Callback from CharacterLocomotion.Move when the UltimateCharacterLocomotion should perform its post movement updates (such as updating the item abilities).
        /// </summary>
        protected override void LateUpdateUltimateLocomotion()
        {
            ...
            if ((!ability.IsActive || ability.CanReceiveMultipleStarts) && ability.Enabled && (ignoreCanStartCheck || ability.CanStartAbility())) {
                ...
                if (!ability.IsActive) {
                    ...
                    if (!(ability is ItemEquipVerifier) && m_ItemEquipVerifierAbility != null) {
                        ...
                        if (m_ItemEquipVerifierAbility.TryToggleItem(ability, true) && !(ability is MoveTowards)) {
                            return true;
                        }
                    }

                    // Insert in the active abilities array according to priority.
                    var insertIndex = m_ActiveAbilityCount;
                    for (int i = 0; i < m_ActiveAbilityCount; ++i) {
                ...
                // Let others know that the ability has started.
                EventHandler.ExecuteEvent(m_GameObject, "OnCharacterAbilityActive", ability, true);
                if (m_OnAbilityActiveEvent != null && m_OnAbilityActiveEvent.GetPersistentEventCount() > 0) {
        ...
        public bool TryStartAbility(ItemAbility itemAbility, bool ignoreCanStartCheck)
        {
            ...
            if ((!itemAbility.IsActive || itemAbility.CanReceiveMultipleStarts) && itemAbility.Enabled && (ignoreCanStartCheck || itemAbility.CanStartAbility())) {
                ...
                if (!itemAbility.IsActive) {
                    ...
                    // Insert in the active abilities array according to priority.
                    var insertIndex = m_ActiveItemAbilityCount;
                    for (int i = 0; i < m_ActiveItemAbilityCount; ++i) {
                    ...
                    for (int i = m_ActiveItemAbilityCount; i > insertIndex; --i) {
                        m_ActiveItemAbilities[i] = m_ActiveItemAbilities[i - 1];
                        m_ActiveItemAbilities[i].ActiveIndex = i;
                    }

                    m_ActiveItemAbilities[insertIndex] = itemAbility;
                    m_ActiveItemAbilityCount++;
                    itemAbility.StartAbility(insertIndex);
                } else {
                    // The ability is already active - start it again for a multiple start.
                    itemAbility.StartAbility(itemAbility.ActiveIndex);
                }
                ...
                if (m_OnItemAbilityActiveEvent != null && m_OnItemAbilityActiveEvent.GetPersistentEventCount() > 0) {
        ...
        public void UpdateAbilityAnimatorParameters(bool immediateUpdate)
        {
            ...
            if (m_MoveTowardsAbility != null && m_MoveTowardsAbility.IsActive) {
                abilityIndex *= -1;
            }
            m_AnimatorMonitor.SetAbilityIndexParameter(abilityIndex);
            m_AnimatorMonitor.SetAbilityIntDataParameter(intData);
            m_AnimatorMonitor.SetAbilityFloatDataParameter(floatData);
        }
        ...
        {
            // ApplyAnimatorMove will be false when the animator is updated outside of when the animations should be updated (such as CharacterIK doing an update pass with a 0 time).
            if (!m_ApplyAnimatorMove) {
                return;
            }
        ...
        public void SetRotation(Quaternion rotation, bool snapAnimator)
        {
            ...
            DeterministicObjectManager.SetCharacterRotation(m_DeterministicObjectIndex, rotation);
        ...
        public void SetPosition(Vector3 position, bool snapAnimator)
        {
            ...
            // If the character isn't active then it isn't registered with the Kinematic Object Manager.
            if (!m_GameObject.activeInHierarchy) {
                return;
            }

            DeterministicObjectManager.SetCharacterPosition(m_DeterministicObjectIndex, position);
        ...
        public override void ResetRotationPosition()
        {
            ...
            DeterministicObjectManager.SetCharacterRotation(m_DeterministicObjectIndex, m_Transform.rotation);
            DeterministicObjectManager.SetCharacterPosition(m_DeterministicObjectIndex, m_Transform.position);
        }
        ...
        public bool SingleCast(Vector3 direction, int layers, ref RaycastHit result)
        {
            for (int i = 0; i < m_Colliders.Length; ++i) {
                ...
                if (m_Colliders[i] is CapsuleCollider) {
                    ...
                    if (Physics.CapsuleCast(startEndCap, endEndCap, radius, direction.normalized, out result, radius + direction.magnitude, layers, QueryTriggerInteraction.Ignore)) {
                        return true;
                    }
                } else { // SphereCollider.
                    var sphereCollider = m_Colliders[i] as SphereCollider;
                    var radius = sphereCollider.radius;
                    if (Physics.SphereCast(sphereCollider.transform.TransformPoint(sphereCollider.center), radius, direction.normalized,
                                                                    out result, radius + direction.magnitude, layers, QueryTriggerInteraction.Ignore)) {
                        return true;
                    }
                }
            }
        ...
        private void OnDeath(Vector3 position, Vector3 force, GameObject attacker)
        {
            ...
            DeterministicObjectManager.SetCharacterMovementInput(m_DeterministicObjectIndex, 0, 0);
            DeterministicObjectManager.SetCharacterDeltaYawRotation(m_DeterministicObjectIndex, 0);
        ...
        private void OnRespawn()
        {
            ...
            if (m_AnimatorMonitor != null) {
                m_AnimatorMonitor.SetPitchParameter(m_LookSource.Pitch, 0);
                m_AnimatorMonitor.SetYawParameter(m_YawAngle, 0);
                m_AnimatorMonitor.SetHorizontalMovementParameter(m_InputVector.x, 0);
                m_AnimatorMonitor.SetForwardMovementParameter(m_InputVector.y, 0);
        ...
        {
            // The DeterministicObjectManager is responsible for calling the move method.
            DeterministicObjectManager.UnregisterCharacter(m_DeterministicObjectIndex);
        }
