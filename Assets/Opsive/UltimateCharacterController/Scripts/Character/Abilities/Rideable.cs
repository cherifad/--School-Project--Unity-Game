...
namespace Opsive.UltimateCharacterController.Character.Abilities
{
    ...
    public class Rideable : Ability
    {
        ...
        protected override void AbilityStarted()
        {
            ...
            // The rideable object must update before the character:
            DeterministicObjectManager.SetCharacterPriorityUpdateOrder(m_CharacterLocomotion.DeterministicObjectIndex, characterLocomotion.DeterministicObjectIndex);
        ...
        private bool DismountColliderOverlap(Collider dismountCollider)
        {
            ...
            if (dismountCollider is CapsuleCollider) {
                ...
                hitCount = Physics.OverlapBoxNonAlloc(dismountCollider.transform.TransformPoint(boxCollider.center), boxCollider.size / 2, m_OverlapColliders, dismountCollider.transform.rotation, m_LayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore);
            } else { // SphereCollider.
                var sphereCollider = dismountCollider as SphereCollider;
                hitCount = Physics.OverlapSphereNonAlloc(dismountCollider.transform.TransformPoint(sphereCollider.center), sphereCollider.radius, m_OverlapColliders, m_LayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore);
            }
