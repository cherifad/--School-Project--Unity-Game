...
namespace Opsive.UltimateCharacterController.Character
{
    ...
    public abstract class CharacterLocomotion : StateBehavior, IForceObject
    {
        ...
        [SerializeField] protected Vector3 m_MotorAirAcceleration = new Vector3(0.18f, 0, 0.18f);
        ...
        [SerializeField] protected float m_MotorAirDamping = 0.27f;
        ...
        [Tooltip("A multiplier to apply to the gravity force.")]
        ...
        [SerializeField] protected LayerMask m_ColliderLayerMask = ~(1 << LayerManager.IgnoreRaycast | 1 << LayerManager.TransparentFX | 
        ...
        [Tooltip("If aligning to ground, specifies the distance away from the ground that the character should align itself to.")]
        [SerializeField] protected float m_AlignToGroundDistance = 2;
        [Tooltip("If aligning to ground, specifies the depth offset when checking the ground normal.")]
        [SerializeField] protected float m_AlignToGroundDepthOffset = 0.05f;
        ...
        [SerializeField] protected bool m_AlignToGroundNormalizeDirection = false;
        [Tooltip("If aligning to ground, specifies the speed that the character can rotate to align to the ground.")]
        [SerializeField] protected float m_AlignToGroundRotationSpeed = 10;
        ...
        public Vector3 MotorAirAcceleration { get { return m_MotorAirAcceleration; } set { m_MotorAirAcceleration = value; } }
        public float MotorAirDamping { get { return m_MotorAirDamping; } set { m_MotorAirDamping = value; } }
        ...
        public float GravityModifier { get { return m_GravityModifier; } set { m_GravityModifier = value; } }
        ...
        public bool AlignToGround { get { return m_AlignToGround; } set { m_AlignToGround = value; } }
        public float AlignToGroundDistance { get { return m_AlignToGroundDistance; } set { m_AlignToGroundDistance = value; } }
        public float AlignToGroundDepthOffset { get { return m_AlignToGroundDepthOffset; } set { m_AlignToGroundDepthOffset = value; } }
        public bool AlignToGroundNormalizeDirection { get { return m_AlignToGroundNormalizeDirection; } set { m_AlignToGroundNormalizeDirection = value; } }
        ...
        [Snapshot] public Vector3 Velocity { get { return m_Velocity; } set { m_Velocity = value; } }
        ...
        protected override void Awake()
        {
            ...
            m_LayerManager = gameObject.GetCachedComponent<LayerManager>();
            ...
            for (int i = 0; i < colliders.Length; ++i) {
                ...
                // The scale must be set to (1, 1, 1).
                if (colliders[i].transform.lossyScale != Vector3.one) {
                    Debug.LogWarning("Warning: The scale on collider " + colliders[i].name + " is not set to (1, 1, 1). This may cause collision issues.");
                }
        ...
        {
            if (m_TimeScale == 0 || Time.fixedDeltaTime == 0) {
                return;
            }
            ...
            // Provide a callback for when the UltimateCharacterLocomotion subclass should perform its post movement updates (such as updating the item abilities).
        ...
        private void UpdatePlatformMovement()
        {
            if (m_Platform == null) {
                ...
                var damping = m_MovingPlatformForceDamping * (m_TimeScale * TimeUtility.FramerateFixedDeltaTime);
            ...
            if (!m_GroundedMovingPlatform) {
                ...
                if (hitCount > 0) {
                    ...
                    for (int i = 0; i < hitCount; ++i) {
                        if (m_CombinedRaycastHits[i].transform == m_Platform) {
            ...
            var platformVelocity = m_PlatformMovement / (m_TimeScale * Time.fixedDeltaTime);
        ...
        protected virtual void UpdateRotation()
        {
            ...
            if (m_AlignToGround) {
                ...
                // adjust to a slope.
                if (m_AlignToGroundDepthOffset != 0) {
                    var frontPoint = m_Transform.position;
                    bool frontHit;
                    if ((frontHit = Physics.Raycast(m_Transform.TransformPoint(0, 0.1f, m_AlignToGroundDepthOffset * Mathf.Sign(m_InputVector.y)),
                                                    -m_Up, out m_RaycastHit, m_AlignToGroundDistance, m_LayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore))) {
                        frontPoint = m_RaycastHit.point;
                        targetNormal = m_RaycastHit.normal;
                    }

                    var backPoint = frontPoint;
                    if (Physics.Raycast(m_Transform.TransformPoint(0, 0.1f, m_AlignToGroundDepthOffset * -Mathf.Sign(m_InputVector.y)),
                                            -m_Up, out m_RaycastHit, m_AlignToGroundDistance, m_LayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore)) {
                        if (frontHit) {
                            if (m_AlignToGroundNormalizeDirection) {
                                backPoint = m_RaycastHit.point;
                                var direction = (frontPoint - backPoint).normalized;
                                targetNormal = Vector3.Cross(direction, Vector3.Cross(m_Up, direction)).normalized;
                            } else {
                                targetNormal = (targetNormal + m_RaycastHit.normal).normalized;
                            }
                        } else {
                            targetNormal = m_RaycastHit.normal;
                        }
                    ...
                    m_GravityDirection = -targetNormal;
                    updateNormalRotation = true;
                } else {
                    var hitCount = NonAllocCast(-m_Up * m_AlignToGroundDistance, m_PlatformMovement + m_Up * c_ColliderSpacing);
                    if (hitCount > 0) {
                        Array.Sort(m_CombinedRaycastHits, 0, hitCount, m_RaycastHitComparer);
                        ...
                        for (int i = 0; i < hitCount; ++i) {
                            ...
                            var activeCollider = m_ColliderCount > 1 ? m_Colliders[m_ColliderIndexMap[m_CombinedRaycastHits[i]]] : m_Colliders[0];
                            if (!MathUtility.IsUnderCollider(m_Transform, activeCollider, m_CombinedRaycastHits[i].point)) {
                                continue;
                            }


                            m_GravityDirection = -m_CombinedRaycastHits[i].normal;
                            updateNormalRotation = true;
                            break;
                        }
                    }
                }
            } else if (-m_Up != m_GravityDirection) {
                targetNormal = -m_GravityDirection;
                updateNormalRotation = true;
            }

            // The rotation is affected by aligning to the ground or having a different up rotation from gravity.
            if (updateNormalRotation) {
                var origRotationDelta = rotationDelta;
                var rotation = m_Transform.rotation * rotationDelta * m_PlatformTorque;
                ...
                if (proj.sqrMagnitude > 0.0001f) {
                    var alignToGroundSpeed = m_Platform == null ? m_AlignToGroundRotationSpeed * m_TimeScale * TimeUtility.FixedDeltaTimeScaled : 1;
                    ...
                    var collisionRotationDelta = CheckRotation(rotationDelta, true);
                    ...
                    rotationDelta = (collisionRotationDelta == rotationDelta ? collisionRotationDelta : origRotationDelta);
                }
            }
        ...
        private Quaternion CheckRotation(Quaternion rotationDelta, bool forceCheck)
        {
                ...
                for (int i = 0; i < m_ColliderCount; ++i) {
                    ...
                    for (int k = 0; k < m_RotationCollisionCheckCount; ++k) {
                        ...
                        if (m_Colliders[i] is CapsuleCollider) {
                            ...
                            hitCount = Physics.OverlapCapsuleNonAlloc(startEndCap, endEndCap, capsuleCollider.radius, m_OverlapColliderHit, m_LayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore);
                        } else { // SphereCollider.
                            var sphereCollider = m_Colliders[i] as SphereCollider;
                            sphereCollider.radius += c_ColliderSpacing;
                            hitCount = Physics.OverlapSphereNonAlloc(targetPosition, sphereCollider.radius, m_OverlapColliderHit, m_LayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore);
                        }
        ...
        protected virtual void UpdateMotorThrottle()
        {
            if (UsingRootMotionPosition || m_LocalRootMotionForce.sqrMagnitude > 0) {
                ...
                var acceleration = (m_Grounded ? m_MotorAcceleration : m_MotorAirAcceleration) * m_SlopeFactor * backwardsMultiplier * 0.1f;
                ...
                // Dampen motor forces.
                m_MotorThrottle /= (1 + ((m_Grounded ? m_MotorDamping : m_MotorAirDamping) * m_TimeScale * Time.timeScale));
            }
        ...
        {
            // The movement should be framerate independent.
            var deltaTime = TimeScaleSquared * TimeUtility.FramerateFixedDeltaTimeScaled;
            var moveDirection = m_ExternalForce + m_MotorThrottle - (m_GravityDirection * m_GravityAmount);
            m_MoveDirection += moveDirection * deltaTime;
        ...
        private void DeflectHorizontalCollisions()
        {
            ...
            if (horizontalDirection.sqrMagnitude < 0.000001f && m_PlatformMovement.sqrMagnitude < 0.000001f) {
            ...
            for (int i = 0; i < hitCount; ++i) {
                ...
                var radius = (activeCollider is CapsuleCollider ? (activeCollider as CapsuleCollider).radius : (activeCollider as SphereCollider).radius);
                var hitGameObject = m_CombinedRaycastHits[i].transform.gameObject;
                var hitRigidbody = hitGameObject.GetCachedParentComponent<Rigidbody>();
                var canStep = true;
                if (hitRigidbody != null) {
                    canStep = !PushRigidbody(hitRigidbody, horizontalDirection, m_CombinedRaycastHits[i].point, radius);
                }
                ...
                if (m_Grounded && canStep) {
                    ...
                    var groundPoint = m_Transform.InverseTransformPoint(m_CombinedRaycastHits[i].point);
                    if (groundPoint.y <= m_MaxStepHeight + c_ColliderSpacing) {
                        ...
                        m_SlopeRay.origin = m_CombinedRaycastHits[i].point - m_SlopeRay.direction * (c_ColliderSpacing + 0.1f);
                        Physics.Raycast(m_SlopeRay, out m_RaycastHit, (c_ColliderSpacing + 0.11f), 1 << hitGameObject.layer, QueryTriggerInteraction.Ignore);
                        ...
                        if (SingleCast(activeCollider, horizontalDirection, m_PlatformMovement + m_Up * (m_MaxStepHeight - c_ColliderSpacing))) {
                            if ((m_RaycastHit.distance - radius - c_ColliderSpacing) < horizontalDirection.magnitude) {
                                // An object was hit. The character can step over the object if the slope is less than the limit.
                                m_SlopeRay.direction = horizontalDirection.normalized;
                                m_SlopeRay.origin = m_CombinedRaycastHits[i].point - m_SlopeRay.direction * (c_ColliderSpacing + 0.1f);
                                Physics.Raycast(m_SlopeRay, out m_RaycastHit, (c_ColliderSpacing + 0.11f), 1 << hitGameObject.layer, QueryTriggerInteraction.Ignore);
                                slope = Vector3.Angle(m_Up, m_RaycastHit.normal);
                                if (slope <= m_SlopeLimit + 0.3f) {
                                    continue;
                                }
                            }
                        } else if (slope >= 89.7f && OverlapCount(activeCollider, horizontalDirection + m_PlatformMovement + m_Up * (m_MaxStepHeight - c_ColliderSpacing)) == 0) {
...
#if UNITY_EDITOR
                Debug.DrawRay(m_CombinedRaycastHits[i].point, m_CombinedRaycastHits[i].normal, Color.red);
#endif

                // Determine the direction that the character should move based off of the angle of the hit object.
                var hitNormal = Vector3.ProjectOnPlane(m_CombinedRaycastHits[i].normal, m_Up).normalized;
                var targetDirection = Vector3.Cross(hitNormal, m_Up).normalized;
                var closestPoint = MathUtility.ClosestPointOnCollider(m_Transform, activeCollider, m_CombinedRaycastHits[i].point, platformIndependentMoveDirection, true, false);
                ...
                moveDistance = Mathf.Min(m_CombinedRaycastHits[i].distance - c_ColliderSpacing, horizontalDirection.magnitude);
                if (moveDistance < 0.001f || Vector3.Angle(m_Up, m_GroundRaycastHit.normal) > m_SlopeLimit + 0.3f) {
                    moveDistance = 0;
                }
                ...
                var dynamicFrictionValue = Mathf.Clamp01(1 - MathUtility.FrictionValue(activeCollider.material, m_CombinedRaycastHits[i].collider.material, true));
                ...
                if (hitMoveDirection.magnitude < 0.001f) {
                ...
                var bouncinessValue = MathUtility.BouncinessValue(activeCollider.material, m_CombinedRaycastHits[i].collider.material);
            ...
            if (hitStrength > 0.0001f) {
                ...
                if (hitCount > 0) {
                    ...
                    for (int i = 0; i < hitCount; ++i) {
                        ...
                        var activeCollider = m_ColliderCount > 1 ? m_Colliders[m_ColliderIndexMap[m_CombinedRaycastHits[i]]] : m_Colliders[0];
                        if (m_Grounded && Vector3.Angle(m_Up, m_GroundRaycastHit.normal) <= m_SlopeLimit + 0.3f) {
                            var groundPoint = m_Transform.InverseTransformPoint(m_CombinedRaycastHits[i].point);
                            if (groundPoint.y <= m_MaxStepHeight + c_ColliderSpacing) {
                                var hitGameObject = m_CombinedRaycastHits[i].transform.gameObject;
                                // A CapsuleCast/SphereCast normal isn't always the true normal: http://answers.unity3d.com/questions/50825/raycasthitnormal-what-does-it-really-return.html.
                                ...
                                if (SingleCast(activeCollider, hitMoveDirection, m_PlatformMovement + m_Up * (m_MaxStepHeight - c_ColliderSpacing))) {
                                    if ((m_RaycastHit.distance - radius - c_ColliderSpacing) < hitMoveDirection.magnitude) {
                                        // An object was hit. The character can step over the object if the slope is less than the limit.
                                        m_SlopeRay.direction = hitMoveDirection.normalized;
                                        m_SlopeRay.origin = m_CombinedRaycastHits[i].point - m_SlopeRay.direction * (c_ColliderSpacing + 0.1f);
                                        Physics.Raycast(m_SlopeRay, out m_RaycastHit, (c_ColliderSpacing + 0.11f), 1 << hitGameObject.layer, QueryTriggerInteraction.Ignore);
                                ...
                                } else if (slope >= 89.7f && OverlapCount(activeCollider, hitMoveDirection + m_PlatformMovement + m_Up * (m_MaxStepHeight - c_ColliderSpacing)) == 0) {
                                    // Step over the object if there are no objects in the way.
                                    continue;
                                }
                            }
...
#if UNITY_EDITOR
                        Debug.DrawRay(m_CombinedRaycastHits[i].point, m_CombinedRaycastHits[i].normal, Color.yellow);
#endif
                        ...
                        var hitMoveDistance = Mathf.Min(m_CombinedRaycastHits[i].distance - moveDistanceContribution - hitMoveDirection.magnitude - c_ColliderSpacing, hitMoveDirection.magnitude);
                        if (hitMoveDistance < 0.001f || Vector3.Angle(m_Up, m_GroundRaycastHit.normal) > m_SlopeLimit + 0.3f) {
                            hitMoveDistance = 0;
                        }
        ...
        private void DeflectVerticalCollisions()
        {
            ...
            if (UsingVerticalCollisionDetection && localMoveDirection.y > 0) {
                ...
                var horizontalDirection = Vector3.ProjectOnPlane(platformIndependentMoveDirection, m_Up);
                for (int i = 0; i < m_ColliderCount; ++i) {
                    if (SingleCast(m_Colliders[i], m_Up * (localMoveDirection.y + c_ColliderSpacing), horizontalDirection)) {
                        // The character would hit an object above it. Use the hit object to determine the max distance that the character can move.
                        localMoveDirection.y = Mathf.Max(0, m_RaycastHit.distance - c_ColliderSpacing);
            ...
            if (accumulateGravity) { // The character is in the air.
                // Accumulate gravity.
                m_GravityAmount += (Physics.gravity.y * m_GravityModifier * 0.001f) / Time.timeScale;
                ...
                if (m_GroundRaycastHit.rigidbody != null) {
                    m_GroundRaycastHit.rigidbody.AddForceAtPosition(-m_Up * ((m_Mass / m_GroundRaycastHit.rigidbody.mass) + m_GravityAmount) / Time.fixedDeltaTime,
        ...
        private bool CheckGround()
        {
            ...
            if (hitCount > 0) {
                ...
                for (int i = 0; i < hitCount; ++i) {
                    ...
                    if (m_CombinedRaycastHits[i].distance == 0 && UsingVerticalCollisionDetection) {
                        ...
                        var overlap = !horizontalStep && UsingHorizontalCollisionDetection && ComputeGroundPenetration(activeCollider, m_CombinedRaycastHits[i].collider, horizontalCastOffset, true, out offset);
                        if (overlap) {
                            overlap = ComputeGroundPenetration(activeCollider, m_CombinedRaycastHits[i].collider, horizontalCastOffset, false, out offset);
                        }
                    ...
                    m_GroundRaycastHit = m_CombinedRaycastHits[i];
                    ...
                    var deltaTime = m_TimeScale * TimeUtility.FramerateFixedDeltaTimeScaled;
        ...
        private bool ComputeGroundPenetration(Collider activeCollider, Collider hitCollider, Vector3 horizontalDirection, bool constantVelocity, out Vector3 offset)
        {
        ...
        protected virtual void ApplyPosition()
        {
            ...
            m_Velocity = (m_Transform.position - m_PrevPosition) / (m_TimeScale * Time.fixedDeltaTime);
        ...
        /// <summary>
        /// Provides a virtual method for the UltimateCharacterLocomotion to perform its post movement updates (such as updating the item abilities).
        /// </summary>
        protected virtual void LateUpdateUltimateLocomotion() { }
        ...
        private void UpdateExternalForces()
        {
            ...
            var deltaTime = m_TimeScale * TimeUtility.FramerateFixedDeltaTime;
        ...
        protected virtual void OnAnimatorMove()
        {
            ...
            if (float.IsNaN(m_AnimatorDeltaPosition.x) || m_TimeScale == 0 || Time.fixedDeltaTime == 0) {
                return;
            }

            if (m_AnimatorDeltaPosition.sqrMagnitude > 0) {
                m_LocalRootMotionForce = m_Transform.InverseTransformDirection(m_AnimatorDeltaPosition) * m_RootMotionSpeedMultiplier / (TimeScaleSquared * TimeUtility.FramerateFixedDeltaTimeScaled);
        ...
        private int NonAllocCast(Vector3 direction, Vector3 offset)
        {
            ...
            for (int i = 0; i < m_ColliderCount; ++i) {
                ...
                if (m_Colliders[i] is CapsuleCollider) {
                    ...
                    localHitCount = Physics.CapsuleCastNonAlloc(startEndCap, endEndCap, radius, direction.normalized, m_RaycastHits, radius + direction.magnitude, m_LayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore);
                } else { // SphereCollider.
                    var sphereCollider = m_Colliders[i] as SphereCollider;
                    ...
                    localHitCount = Physics.SphereCastNonAlloc(sphereCollider.transform.TransformPoint(sphereCollider.center) + offset, radius, direction.normalized,
                                                                    m_RaycastHits, direction.magnitude, m_LayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore);
                }
        ...
        private bool SingleCast(Collider collider, Vector3 direction, Vector3 offset)
        {
            ...
            if (collider is CapsuleCollider) {
                ...
                MathUtility.CapsuleColliderEndCaps(capsuleCollider, capsuleCollider.transform.position + offset, capsuleCollider.transform.rotation, out startEndCap, out endEndCap);
                var radius = capsuleCollider.radius;
                return Physics.CapsuleCast(startEndCap, endEndCap, radius, direction.normalized, out m_RaycastHit, radius + direction.magnitude, m_LayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore);
            } else { // SphereCollider.
                ...
                return Physics.SphereCast(sphereCollider.transform.TransformPoint(sphereCollider.center) + offset, radius, direction.normalized,
                                                                out m_RaycastHit, radius + direction.magnitude, m_LayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore);
            }
        }
        ...
        private int OverlapCount(Collider collider, Vector3 offset)
        {
            if (collider is CapsuleCollider) {
                ...
                return Physics.OverlapCapsuleNonAlloc(startEndCap, endEndCap, capsuleCollider.radius, m_OverlapColliderHit, m_LayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore);
            } else { // SphereCollider.
                var sphereCollider = collider as SphereCollider;
                return Physics.OverlapSphereNonAlloc(sphereCollider.transform.TransformPoint(sphereCollider.center) + offset, sphereCollider.radius, m_OverlapColliderHit, m_LayerManager.SolidObjectLayers, QueryTriggerInteraction.Ignore);
            }
        }
        ...
        {
            m_ColliderCount = AddColliders(ref m_Colliders, ref m_ColliderLayers, ref m_ColliderGameObjects, m_ColliderCount, colliders);
        ...
        private int AddColliders(ref Collider[] existingColliders, ref int[] existingColliderLayers, ref GameObject[] existingColliderGameObjects, int existingColliderCount, Collider[] colliders)
        {
        ...
        {
            m_ColliderCount = RemoveColliders(ref m_Colliders, ref m_ColliderGameObjects, m_ColliderCount, colliders);
        }
        ...
        private int RemoveColliders(ref Collider[] existingColliders, ref GameObject[] existingColliderGameObjects, int existingColliderCount, Collider[] colliders)
        {
        ...
        public void AddSubCollider(Collider collider)
        {
            // Don't add an already added collider.
            for (int i = 0; i < m_SubColliderCount; ++i) {
                if (m_SubColliders[i] == collider) {
                    return;
                }
            }
            // The collider should be added.
            if (m_SubColliderCount == m_SubColliders.Length) {
                Array.Resize(ref m_SubColliders, m_SubColliders.Length + 1);
                Array.Resize(ref m_SubColliderLayers, m_SubColliderLayers.Length + 1);
                Array.Resize(ref m_SubColliderGameObjects, m_SubColliderGameObjects.Length + 1);
            }
            m_SubColliders[m_SubColliderCount] = collider;
            m_SubColliderGameObjects[m_SubColliderCount] = collider.gameObject;
            m_SubColliderCount++;
        }
        ...
        {
            for (int i = m_SubColliders.Length - 1; i > -1; --i) {
                if (m_SubColliders[i] != collider) {
                    continue;
                }

                // Do not resize the array for performance reasons. Move all of the next colliders back a slot instead.
                for (int j = i; j < m_SubColliderCount - 1; ++j) {
                    m_SubColliders[j] = m_SubColliders[j + 1];
                    m_SubColliderGameObjects[j] = m_SubColliderGameObjects[j + 1];
                }
                m_SubColliderCount--;
                m_SubColliders[i] = null;
                m_SubColliderGameObjects[i] = null;
                break;
            }
        }
        ...
        {
            m_SubColliderCount = AddColliders(ref m_SubColliders, ref m_SubColliderLayers, ref m_SubColliderGameObjects, m_SubColliderCount, colliders);
        }
        ...
        {
            m_SubColliderCount = RemoveColliders(ref m_SubColliders, ref m_SubColliderGameObjects, m_SubColliderCount, colliders);
        }
        ...
        public virtual void SetPosition(Vector3 position)
        {
            ...
            // Update the grounded state based on the current position.
            EnableColliderCollisionLayer(false);
            UpdateGroundState(CheckGround(), true);
            EnableColliderCollisionLayer(true);
        }
