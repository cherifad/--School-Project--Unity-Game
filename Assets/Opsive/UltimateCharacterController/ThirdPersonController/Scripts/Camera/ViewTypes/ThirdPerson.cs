...
namespace Opsive.UltimateCharacterController.ThirdPersonController.Camera.ViewTypes
{
    ...
    public abstract class ThirdPerson : ViewType
    {
        ...
        public override Vector3 Move(bool immediateUpdate)
        {
            ...
            var direction = lookPosition - anchorPosition;
            m_CharacterLocomotion.EnableColliderCollisionLayer(false);
            // Fire a sphere to prevent the camera from colliding with other objects.
            if (Physics.SphereCast(anchorPosition - direction.normalized * m_CollisionRadius, m_CollisionRadius, direction.normalized, out m_RaycastHit, direction.magnitude, 
                ...
                if (localDirection.y > 0) {
                    ...
                    if (!Physics.SphereCast(anchorPosition, m_CollisionRadius, direction.normalized, out m_RaycastHit, direction.magnitude - m_CollisionRadius, m_CharacterLayerManager.IgnoreInvisibleCharacterWaterLayers, QueryTriggerInteraction.Ignore)) {
            ...
            if (m_Camera.fieldOfView != m_FieldOfView) {
                ...
                if (m_CameraController.ActiveViewType == this) {
                    ...
                    if (m_PositionSmoothing == 0 || (m_PrevPositionSmoothing == 0 && m_PositionSmoothing != m_PrevPositionSmoothing)) {
                        DeterministicObjectManager.SetCameraPosition(m_CameraController.DeterministicObjectIndex, (m_Transform.position + Move(true)) * 0.5f);
                    }
                }
            }
