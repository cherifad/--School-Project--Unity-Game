...
namespace Opsive.UltimateCharacterController.ThirdPersonController.Character
{
    ...
    public class PerspectiveMonitor : StateBehavior
    {
        ...
        public Material InvisibleMaterial { get { return m_InvisibleMaterial; } }
        ...
        private List<GameObject> m_RendererGameObjects = new List<GameObject>();
        ...
        protected override void Awake()
        {
            ...
            if (characterRenderers != null) {
                for (int i = 0; i < characterRenderers.Length; ++i) {
                    ...
                    CacheRendererMaterials(renderer, renderer.gameObject.GetCachedInactiveComponentInParent<ThirdPersonObject>());
                }
            }
        ...
        /// <param name="thirdPersonRenderer">Does the renderer have the ThirdPersonObject component attached?</param>
        private void CacheRendererMaterials(Renderer renderer, bool thirdPersonRenderer)
        {
            if (thirdPersonRenderer) {
                m_ThirdPersonRenderers.Add(m_Renderers.Count);
            }

            m_Renderers.Add(renderer);
            m_RendererGameObjects.Add(renderer.gameObject);
        ...
        private void OnChangePerspectives(bool firstPersonPerspective)
        {
            ...
            for (int i = 0; i < m_ThirdPersonRenderers.Count; ++i) {
                ...
                m_Renderers[thirdPersonIndex].materials = (firstPersonPerspective ? m_InvisibleMaterials[thirdPersonIndex] : m_OriginalMaterials[thirdPersonIndex]);
            }
                ...
                if (thirdPersonObject != null) {
                    ...
                    for (int i = 0; i < renderers.Length; ++i) {
                        if (!m_RegisteredRenderers.Contains(renderers[i])) {
                            CacheRendererMaterials(renderers[i], true);
        ...
        private bool TryUpdateDeathMaterials(bool fromDeathEvent)
        {
            ...
            for (int i = 0; i < m_Renderers.Count; ++i) {
                ...
                if (m_DeathVisibility == ObjectDeathVisiblity.AllInvisible) {
                    ...
                    var thirdPersonObject = m_RendererGameObjects[i].GetCachedParentComponent<ThirdPersonObject>();
                    invisibleObject = m_CharacterLocomotion.FirstPersonPerspective && (thirdPersonObject != null && !thirdPersonObject.FirstPersonVisibleOnDeath);
                }
        ...
        private void OnRespawn()
        {
            ...
            for (int i = 0; i < m_Renderers.Count; ++i) {
                m_Renderers[i].materials = (!m_CharacterLocomotion.FirstPersonPerspective || 
                    (m_RendererGameObjects[i].GetCachedInactiveComponentInParent<ThirdPersonObject>() == null)) ? m_OriginalMaterials[i] : m_InvisibleMaterials[i];
            }
        }
